...
public class LambdaExample {
    ...
    public static void main(final String[] args) {
        ...
        Predicate<User> olderThanFive= new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age>5;
            }
        };
        repository.select(olderThanFive,byName);
        ...
        // DONE
        Predicate<User> lessThanTen= new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age<10;
            }
        };
        repository.select(lessThanTen,byAge);
        ...
        // DONE
        Predicate<User> activeUserAndOlderThanEight= new Predicate<User>(){
            @Override
            public boolean test(User user){
                return user.age> 8 && user.active;
            }
        };
        repository.select(activeUserAndOlderThanEight,byName);
